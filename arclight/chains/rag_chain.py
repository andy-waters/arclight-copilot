...
def _demo_docs(q: str) -> List[Doc]:
    return [
        Doc(
            source="/docs/azure_rag.pdf#p1",
            content="Azure AI Search enables semantic and vector retrieval.",
        ),
        Doc(
            source="/docs/agentic_patterns.md",
            content="Planner → Researcher → Reviewer reflects and retries.",
        ),
    ]
...
def _format_ctx(docs: List[dict]) -> str:
    return "\n\n".join(
        [
            f"[{i+1}] {d.get('source')}: {d.get('content')[:700]}"
...
def _search_azure(query: str, k: int = 6) -> List[dict]:
    ...
    try:
        ...
        results = sc.search(
            search_text=None,
            vector=Vector(value=vec, k=k, fields="embedding"),
